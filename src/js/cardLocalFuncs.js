...
function inputOrCoord(inputValue, choice, coordTz) {
    ...
    let res;
    if (choice === 'input') {
        res = countryTimezone.getTimezones(inputValue)[Number(0)]; // it is necessary to convert index to number so eslint wont throw destructuring error
    } else if (choice === 'map') {
        res = coordTz;
    }
    console.log(res);
...
function getLastLetterNum(string) {
    ...
    for (let i = -1; i > -Math.abs(string.length); i += -1) {
        ...
        if (string.slice(i)[0].match(/[a-zA-Z0-9]/)) {
            // string.slice(i)[0] returns the ith character
            if (i === -1) {
